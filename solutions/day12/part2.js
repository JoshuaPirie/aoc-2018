...
let currCount = Array.from(currState.potPlants.values()).reduce((acc, curr) => acc + curr, 0);
let prevDiff = null;
let iterations = 0;
...
while(true) {
    iterate();
    iterations++;
    const newCount = Array.from(currState.potPlants.values()).reduce((acc, curr) => acc + curr, 0);
...
console.log(currCount + (50000000000 - iterations) * prevDiff);
